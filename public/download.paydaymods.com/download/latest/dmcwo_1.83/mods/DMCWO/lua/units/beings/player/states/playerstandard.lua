...
function PlayerStandard:_start_action_running(t)
	...
	if self._shooting and not managers.player.RUN_AND_SHOOT or self:_changing_weapon() or self:_is_meleeing() or self._use_item_expire_t or self._state_data.in_air or self:_is_throwing_projectile() or self:_is_charging_weapon() then
		self._running_wanted = true
		return
	end
	...
	if not self:_is_reloading() or not self.RUN_AND_RELOAD then
		...
		elseif not self.RUN_AND_RELOAD and not self._bolt_time then
...
function PlayerStandard:_check_action_steelsight(t, input)
	...
	if (self._end_running_expire_t and not managers.player.RUN_AND_SHOOT) or (self._bolt_time and weap_base._bolt_time_unscope) or (weap_base:in_burst_mode() and weap_base._no_ads_burst == true) or (not weap_base:in_burst_mode() and fire_mode == "auto" and weap_base._no_ads_auto == true) then
		self:_interupt_action_steelsight(t)
		if input.btn_steelsight_press or input.btn_steelsight_state then
...
function PlayerStandard:update(t, dt)
	...
	local action_forbidden = self:_is_reloading() or self:_is_cash_inspecting(t) or self:_changing_weapon() or self:_is_meleeing() or self._use_item_expire_t or self:_interacting() or self:_is_throwing_projectile() or self:_is_deploying_bipod() or self._melee_equip_weapon_expire_t or ( (self._running or self._end_running_expire_t) and not managers.player.RUN_AND_SHOOT)
	...
	if self._bolt_time then
		self:_update_bolt_time(t, dt)
	end
	if self._bolt_delay then
		self:_update_bolt_delay(t, dt)
	end
	if not action_forbidden and weap_base._must_bolt and not self._bolt_time then
		self._bolt_time = weap_base._bolt_time * (weap_base:weapon_tweak_data().fire_mode_data.fire_rate / weap_base:fire_rate_multiplier())
		self._bolt_delay = weap_base:weapon_tweak_data().fire_mode_data.fire_rate / weap_base:fire_rate_multiplier()
		self:_play_bolt_anim(t, dt)
	end
...
function PlayerStandard:_play_bolt_anim(t, dt)
	local weap_base = self._equipped_unit:base()
	local fire_mode = weap_base:fire_mode()
	local anim_speed_mult = weap_base:fire_rate_multiplier() or 1
	...
	if weap_base._bolt_time_ads_anim then
	...
	if weap_base._bolt_time_ads_anim then
		self._ext_camera:play_redirect(self.IDS_RECOIL_STEELSIGHT, anim_speed_mult or 1)
	else
		self._ext_camera:play_redirect(self.IDS_RECOIL, anim_speed_mult or 1)
	end
end	
		
function PlayerStandard:_update_bolt_time(t, dt)
	local weap_base = self._equipped_unit:base()
	if self._bolt_time then
		self._bolt_time = self._bolt_time - dt
		if self._bolt_time < 0 then
			self._bolt_time = nil
			weap_base._must_bolt = nil
			log("BOLT DONE")
		end
	end
end	

function PlayerStandard:_update_bolt_delay(t, dt)
	local weap_base = self._equipped_unit:base()
	if self._bolt_delay then
		self._bolt_delay = self._bolt_delay - dt
		if self._bolt_delay < 0 then
			self._bolt_delay = nil
		end
	end
end	
...
function PlayerStandard:_interupt_action_reload(t)
	...
	local speed_multiplier = self._equipped_unit:base():reload_speed_multiplier()
	if t and self._state_data.reload_expire_t and self._equipped_unit:base():reload_exit_expire_t() and self._equipped_unit:base():reload_not_empty_exit_expire_t() then
...
function PlayerStandard:_update_reload_timers(t, dt, input)
	...
	if self._state_data.reload_expire_t then
		...
		if t >= self._state_data.reload_expire_t or interupt then
			managers.player:remove_property("shock_and_awe_reload_multiplier")
			self._state_data.reload_expire_t = nil
			if self._equipped_unit:base()._bolt_time_reload then
				self._bolt_time = nil
				self._equipped_unit:base()._must_bolt = nil
			end
			...
			if self._equipped_unit then
				...
				elseif weap_base.clip_empty and weap_base:clip_empty() then
					...
					elseif fire_mode == "single" then
						if input.btn_primary_attack_press then
							if weap_base:weapon_tweak_data().category == "bow" then
				...
				elseif self._running and not managers.player.RUN_AND_SHOOT then
					...
					if fired then
						...
						if weap_base._bolt_time then
							if not weap_base:clip_empty() then
								weap_base._must_bolt = true
							end
							self._bolt_time = weap_base._bolt_time * (weap_base:weapon_tweak_data().fire_mode_data.fire_rate / weap_base:fire_rate_multiplier())
							if input.btn_steelsight_state then
								self._steelsight_wanted = true
